// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design implementation internals
// See Vtinyml_accelerator.h for the primary calling header

#include "Vtinyml_accelerator__pch.h"
#include "Vtinyml_accelerator___024root.h"

VL_INLINE_OPT void Vtinyml_accelerator___024root___ico_sequent__TOP__0(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___ico_sequent__TOP__0\n"); );
    // Body
    vlSelf->tinyml_accelerator__DOT__sum = (vlSelf->tinyml_accelerator__DOT__accumulator 
                                            + (vlSelf->pcpi_rs1 
                                               * vlSelf->pcpi_rs2));
    vlSelf->tinyml_accelerator__DOT__next_result = 
        ((IData)(((0x1000U == (0x7000U & vlSelf->pcpi_insn)) 
                  & (vlSelf->tinyml_accelerator__DOT__sum 
                     >> 0x1fU))) ? 0U : vlSelf->tinyml_accelerator__DOT__sum);
}

void Vtinyml_accelerator___024root___eval_ico(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___eval_ico\n"); );
    // Body
    if ((1ULL & vlSelf->__VicoTriggered.word(0U))) {
        Vtinyml_accelerator___024root___ico_sequent__TOP__0(vlSelf);
    }
}

void Vtinyml_accelerator___024root___eval_triggers__ico(Vtinyml_accelerator___024root* vlSelf);

bool Vtinyml_accelerator___024root___eval_phase__ico(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___eval_phase__ico\n"); );
    // Init
    CData/*0:0*/ __VicoExecute;
    // Body
    Vtinyml_accelerator___024root___eval_triggers__ico(vlSelf);
    __VicoExecute = vlSelf->__VicoTriggered.any();
    if (__VicoExecute) {
        Vtinyml_accelerator___024root___eval_ico(vlSelf);
    }
    return (__VicoExecute);
}

void Vtinyml_accelerator___024root___eval_act(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___eval_act\n"); );
}

VL_INLINE_OPT void Vtinyml_accelerator___024root___nba_sequent__TOP__0(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___nba_sequent__TOP__0\n"); );
    // Body
    vlSelf->pcpi_wait = 0U;
    if (vlSelf->resetn) {
        vlSelf->pcpi_ready = 0U;
        vlSelf->pcpi_wr = 0U;
        if (((IData)(vlSelf->pcpi_valid) & (0xbU == 
                                            (0x7fU 
                                             & vlSelf->pcpi_insn)))) {
            vlSelf->pcpi_ready = 1U;
            vlSelf->pcpi_wr = 1U;
            vlSelf->pcpi_rd = vlSelf->tinyml_accelerator__DOT__next_result;
            vlSelf->tinyml_accelerator__DOT__accumulator 
                = vlSelf->tinyml_accelerator__DOT__next_result;
        }
    } else {
        vlSelf->pcpi_ready = 0U;
        vlSelf->pcpi_wr = 0U;
        vlSelf->pcpi_rd = 0U;
        vlSelf->tinyml_accelerator__DOT__accumulator = 0U;
    }
    vlSelf->tinyml_accelerator__DOT__sum = (vlSelf->tinyml_accelerator__DOT__accumulator 
                                            + (vlSelf->pcpi_rs1 
                                               * vlSelf->pcpi_rs2));
    vlSelf->tinyml_accelerator__DOT__next_result = 
        ((IData)(((0x1000U == (0x7000U & vlSelf->pcpi_insn)) 
                  & (vlSelf->tinyml_accelerator__DOT__sum 
                     >> 0x1fU))) ? 0U : vlSelf->tinyml_accelerator__DOT__sum);
}

void Vtinyml_accelerator___024root___eval_nba(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___eval_nba\n"); );
    // Body
    if ((1ULL & vlSelf->__VnbaTriggered.word(0U))) {
        Vtinyml_accelerator___024root___nba_sequent__TOP__0(vlSelf);
    }
}

void Vtinyml_accelerator___024root___eval_triggers__act(Vtinyml_accelerator___024root* vlSelf);

bool Vtinyml_accelerator___024root___eval_phase__act(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___eval_phase__act\n"); );
    // Init
    VlTriggerVec<1> __VpreTriggered;
    CData/*0:0*/ __VactExecute;
    // Body
    Vtinyml_accelerator___024root___eval_triggers__act(vlSelf);
    __VactExecute = vlSelf->__VactTriggered.any();
    if (__VactExecute) {
        __VpreTriggered.andNot(vlSelf->__VactTriggered, vlSelf->__VnbaTriggered);
        vlSelf->__VnbaTriggered.thisOr(vlSelf->__VactTriggered);
        Vtinyml_accelerator___024root___eval_act(vlSelf);
    }
    return (__VactExecute);
}

bool Vtinyml_accelerator___024root___eval_phase__nba(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___eval_phase__nba\n"); );
    // Init
    CData/*0:0*/ __VnbaExecute;
    // Body
    __VnbaExecute = vlSelf->__VnbaTriggered.any();
    if (__VnbaExecute) {
        Vtinyml_accelerator___024root___eval_nba(vlSelf);
        vlSelf->__VnbaTriggered.clear();
    }
    return (__VnbaExecute);
}

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtinyml_accelerator___024root___dump_triggers__ico(Vtinyml_accelerator___024root* vlSelf);
#endif  // VL_DEBUG
#ifdef VL_DEBUG
VL_ATTR_COLD void Vtinyml_accelerator___024root___dump_triggers__nba(Vtinyml_accelerator___024root* vlSelf);
#endif  // VL_DEBUG
#ifdef VL_DEBUG
VL_ATTR_COLD void Vtinyml_accelerator___024root___dump_triggers__act(Vtinyml_accelerator___024root* vlSelf);
#endif  // VL_DEBUG

void Vtinyml_accelerator___024root___eval(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___eval\n"); );
    // Init
    IData/*31:0*/ __VicoIterCount;
    CData/*0:0*/ __VicoContinue;
    IData/*31:0*/ __VnbaIterCount;
    CData/*0:0*/ __VnbaContinue;
    // Body
    __VicoIterCount = 0U;
    vlSelf->__VicoFirstIteration = 1U;
    __VicoContinue = 1U;
    while (__VicoContinue) {
        if (VL_UNLIKELY((0x64U < __VicoIterCount))) {
#ifdef VL_DEBUG
            Vtinyml_accelerator___024root___dump_triggers__ico(vlSelf);
#endif
            VL_FATAL_MT("rtl/tinyml_accelerator.v", 1, "", "Input combinational region did not converge.");
        }
        __VicoIterCount = ((IData)(1U) + __VicoIterCount);
        __VicoContinue = 0U;
        if (Vtinyml_accelerator___024root___eval_phase__ico(vlSelf)) {
            __VicoContinue = 1U;
        }
        vlSelf->__VicoFirstIteration = 0U;
    }
    __VnbaIterCount = 0U;
    __VnbaContinue = 1U;
    while (__VnbaContinue) {
        if (VL_UNLIKELY((0x64U < __VnbaIterCount))) {
#ifdef VL_DEBUG
            Vtinyml_accelerator___024root___dump_triggers__nba(vlSelf);
#endif
            VL_FATAL_MT("rtl/tinyml_accelerator.v", 1, "", "NBA region did not converge.");
        }
        __VnbaIterCount = ((IData)(1U) + __VnbaIterCount);
        __VnbaContinue = 0U;
        vlSelf->__VactIterCount = 0U;
        vlSelf->__VactContinue = 1U;
        while (vlSelf->__VactContinue) {
            if (VL_UNLIKELY((0x64U < vlSelf->__VactIterCount))) {
#ifdef VL_DEBUG
                Vtinyml_accelerator___024root___dump_triggers__act(vlSelf);
#endif
                VL_FATAL_MT("rtl/tinyml_accelerator.v", 1, "", "Active region did not converge.");
            }
            vlSelf->__VactIterCount = ((IData)(1U) 
                                       + vlSelf->__VactIterCount);
            vlSelf->__VactContinue = 0U;
            if (Vtinyml_accelerator___024root___eval_phase__act(vlSelf)) {
                vlSelf->__VactContinue = 1U;
            }
        }
        if (Vtinyml_accelerator___024root___eval_phase__nba(vlSelf)) {
            __VnbaContinue = 1U;
        }
    }
}

#ifdef VL_DEBUG
void Vtinyml_accelerator___024root___eval_debug_assertions(Vtinyml_accelerator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtinyml_accelerator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtinyml_accelerator___024root___eval_debug_assertions\n"); );
    // Body
    if (VL_UNLIKELY((vlSelf->clk & 0xfeU))) {
        Verilated::overWidthError("clk");}
    if (VL_UNLIKELY((vlSelf->resetn & 0xfeU))) {
        Verilated::overWidthError("resetn");}
    if (VL_UNLIKELY((vlSelf->pcpi_valid & 0xfeU))) {
        Verilated::overWidthError("pcpi_valid");}
}
#endif  // VL_DEBUG
